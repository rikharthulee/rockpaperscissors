...
function getPlayerInput() {
  ...
  if (options.includes(playerSelection)) {
    //Steve has tidied up code here.  No need for 3 if statements.
...
function game() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    console.log(
      `Round ${
        i + 1
      }: ${playerName} chose ${playerSelection}, Computer chose ${computerSelection}, Result: ${displayResult}`
    ); //need to learn exactly what Steve has done here
  }

  console.log(
    `Final Score - ${playerName}: ${playerScore}, Computer: ${computerScore}`
  );
